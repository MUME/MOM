...
Public Function checkTheMap(ByRef theCount As Long, ByRef theMap As Long, ByRef theExitType As Long, ByVal row As Long, ByVal col As Long, ByRef data As String)
  ...
  If debug_mode = True Then Debug.Print "______________________Remove_Room_Move"
   ...
   Next
   If (arr(virtualRow, virtualCol) And theMap) > 0 And (theCount < arrMaxRoom) Then 'some kind of exit
      checkTheMap = False
      Select Case theExitType
...
Public Sub updateTheRoom()
   If arrRoomStack(1, 1) > 0 And arrRoomStack(1, 2) > 0 Then
      If roomCount >= limit Then BestEST.tcpClient.SendData arrMoveStack(limit) & vbLf  '¤¤¤¤¤¤¤
...
Public Function checkTheMove(ByRef theCount As Long, ByRef command As String)
   ...
   Select Case command
   Case "n"
      If checkTheMap(theCount, N_map, N_exit, virtualRow - 1, virtualCol, "n") = True Then
         checkTheMove = True
      End If
   Case "e"
      If checkTheMap(theCount, E_map, E_exit, virtualRow, virtualCol + 1, "e") = True Then
         checkTheMove = True
      End If
   Case "s"
      If checkTheMap(theCount, S_map, S_exit, virtualRow + 1, virtualCol, "s") = True Then
         checkTheMove = True
      End If
   Case "w"
      If checkTheMap(theCount, W_map, W_exit, virtualRow, virtualCol - 1, "w") = True Then
         checkTheMove = True
      End If
   Case "u"
      If checkTheMap(theCount, U_map, U_exit, virtualRow, virtualCol, "u") = True Then
         checkTheMove = True
      End If
   Case "d"
      If checkTheMap(theCount, D_map, D_exit, virtualRow, virtualCol, "d") = True Then
         checkTheMove = True
      End If
   End Select
End Function
